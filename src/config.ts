...
export const BOOTSTRAP_URL = __DEV__ ? localBootstrap : remoteBootstrap;
export const SIMULATE = Boolean(__DEV__);

export let API_URL = `${BOOTSTRAP_URL}/api/v1`;
...
export const setApiUrl = (domain: string): void => {
  API_URL = `${domain}/api/v1`;
  console.info(`API URL actualizado: ${API_URL}`);
};
