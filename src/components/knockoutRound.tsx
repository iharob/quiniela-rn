...
export const KnockoutRoundView: React.FC<Props> = observer(
  (props: Props): React.ReactElement => {
    const { games, round } = props;

    const navigation =
      useNavigation<NativeStackNavigationProp<PredictionStackParamsList>>();
    ...
    const total = games.reduce(
      (accumulator: number, _: [GameWithResult, GameWithResult]): number =>
        accumulator + 2,
      0,
    );

    const completed = pairs.reduce(
      (accumulator: number, pair: [GameWithResult, GameWithResult]): number => {
        ...
        return (
          accumulator +
          (winner1.country === 'NA' ? 0 : 1) +
          (winner2.country === 'NA' ? 0 : 1)
        );
      },
      0,
    );
    ...
    const handleNext = React.useCallback((): void => {
      ...
      const roundAfterNext = findRound(
        knockoutStore.bracketFixtures,
        nextApiRound - 1,
      );
      if (!roundAfterNext) {
        console.error(
          `No bracket fixtures found for round ${nextApiRound - 1}`,
        );
        return;
      }

      const nextRoundPairs = pairGamesByFeeders(
        nextGames,
        roundAfterNext.matches,
      );
      const simulatedPairs = SIMULATE
        ? nextRoundPairs.map(simulatePair)
        : nextRoundPairs;
    ...
    React.useEffect((): void => {
      const currentRound = rounds[total] ?? { name: '-' };

      navigation.setOptions({
        title: `${currentRound.name} ${completed}/${total}`,
      });
